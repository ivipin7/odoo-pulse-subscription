...
export const SubscriptionRepository = {
  ...
  async create(data: any) {
    const result = await pool.query(
      `INSERT INTO subscriptions (user_id, product_id, variant_id, status, billing_period, amount, start_date, next_billing)
       VALUES ($1, $2, $3, 'DRAFT', $4, $5, $6, $6) RETURNING *`,
      [data.user_id, data.product_id, data.variant_id || null, data.billing_period, data.amount, data.start_date || new Date().toISOString().split('T')[0]]
    );
