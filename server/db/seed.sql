-- ============================================================
-- OdooPulse — Seed Data
-- Matches frontend mock data (src/data/mockData.ts)
...
VALUES
  ...
  -- Customer users (from subscriptions & userProfile mock)
...
-- ============================================================
-- 2. ADDRESSES (for userProfile — Rajesh Kumar = user_id 7)
-- ============================================================
...
-- ============================================================
-- 4. PRODUCTS (matches mock products array)
-- ============================================================
...
-- Reset the sequence to avoid conflict with future inserts
SELECT setval('products_id_seq', (SELECT MAX(id) FROM products));

-- ============================================================
-- 5. PRODUCT VARIANTS (matches mock variants)
-- ============================================================
...
-- ============================================================
-- 6. SUBSCRIPTIONS (matches mock subscriptions array)
-- ============================================================
-- User mapping:
--   Acme Corp       = user_id 7  (Rajesh Kumar)
--   TechStart Inc   = user_id 8  (Amit Patel)
--   GlobalTrade Ltd = user_id 9  (Sarah Khan)
--   FinanceHub      = user_id 10 (Deepak Joshi)
--   RetailMax       = user_id 11 (Meera Reddy)
--   MediaFlow       = user_id 12 (Kiran Rao)
--   BuildRight Co   = user_id 13 (Ravi Nair)
--   DataDriven LLC  = user_id 14 (Pooja Verma)

INSERT INTO subscriptions (id, user_id, product_id, variant_id, status, start_date, next_billing, billing_period, amount)
VALUES
  (1,  7,  1,  3,  'ACTIVE',   '2024-06-15', '2025-03-15', 'MONTHLY', 2400.00),   -- SUB-001: Acme Corp, ERP Suite Enterprise
  (2,  8,  2,  5,  'ACTIVE',   '2024-09-01', '2025-03-01', 'MONTHLY', 1200.00),   -- SUB-002: TechStart Inc, CRM Pro Business
  (3,  9,  5, 11,  'AT_RISK',  '2024-03-20', '2025-02-20', 'MONTHLY', 1200.00),   -- SUB-003: GlobalTrade Ltd, Inventory Control Multi
  (4, 10,  4,  9,  'ACTIVE',   '2024-11-10', '2025-05-10', 'MONTHLY', 1500.00),   -- SUB-004: FinanceHub, Accounting Plus Advanced
  (5, 11,  1,  2,  'AT_RISK',  '2024-07-01', '2025-02-01', 'MONTHLY', 1700.00),   -- SUB-005: RetailMax, ERP Suite Professional
  (6, 12,  6, 13,  'CLOSED',   '2024-01-15', NULL,         'MONTHLY',  850.00),   -- SUB-006: MediaFlow, Marketing Hub Growth
  (7, 13,  3,  7,  'ACTIVE',   '2024-10-20', '2025-04-20', 'MONTHLY',  900.00),   -- SUB-007: BuildRight Co, HR Management Premium
  (8, 14,  2,  4,  'CLOSED',   '2024-02-10', NULL,         'MONTHLY',  800.00);   -- SUB-008: DataDriven LLC, CRM Pro Starter

-- Mark closed subscription timestamps
...
-- ============================================================
-- 7. INVOICES (matches mock invoices array)
-- ============================================================
...
-- ============================================================
-- 8. PAYMENTS (matches mock payments array)
-- ============================================================
...
-- ============================================================
-- 9. PAYMENT RETRIES (audit log for Siva's recovery engine)
-- ============================================================
-- GlobalTrade Ltd (invoice 3) — 3 retries, all FAILED → subscription CLOSED-worthy
-- But per mock, subscription is AT_RISK (let's keep 3 retries as FAILED to match)

INSERT INTO payment_retries (invoice_id, payment_id, attempt_number, status, failure_reason, attempted_at)
VALUES
  -- GlobalTrade Ltd invoice retries (INV-2025-003, invoice_id=3)
  ...
  -- RetailMax invoice retries (INV-2025-005, invoice_id=5)
  (5, NULL, 1, 'FAILED', 'Card expired',          '2025-02-04 14:00:00+05:30'),
  (5, NULL, 2, 'FAILED', 'Card declined',         '2025-02-08 14:30:00+05:30');

-- ============================================================
-- 10. ORDERS (matches mock orders array)
-- ============================================================
-- All orders belong to Rajesh Kumar (user_id 7) as the portal user
...
-- ============================================================
-- 11. ORDER ITEMS (matches mock order items)
-- ============================================================

INSERT INTO order_items (order_id, product_id, variant_id, quantity, unit_price, billing_period)
VALUES
  (1, 1, 2,  1, 1700.00, 'MONTHLY'),   -- ORD-001: ERP Suite Professional
  (2, 2, 5,  1, 1200.00, 'MONTHLY'),   -- ORD-002: CRM Pro Business
  (3, 3, 7,  1,  900.00, 'MONTHLY'),   -- ORD-003: HR Management Premium
  (4, 4, 8,  1,  900.00, 'MONTHLY'),   -- ORD-004: Accounting Plus Standard
  (5, 6, 13, 2,  850.00, 'MONTHLY');   -- ORD-005: Marketing Hub Growth x2

-- ============================================================
-- 12. CART ITEMS (matches mock initialCartItems for Rajesh Kumar)
-- ============================================================

INSERT INTO cart_items (user_id, product_id, variant_id, quantity)
VALUES
  (7, 1, 2,  1),   -- ERP Suite Professional
  (7, 3, 7,  1),   -- HR Management Premium
  (7, 6, 12, 2);   -- Marketing Hub Essentials x2

-- ============================================================
-- 13. QUOTATIONS (matches mock quotations array)
-- ============================================================

INSERT INTO quotations (id, quotation_number, user_id, total_amount, valid_until, status)
VALUES
  (1, 'QOT-001', 15, 34800.00, '2025-03-15', 'SENT'),      -- NovaTech Systems (user_id 15)
  (2, 'QOT-002', 16, 10800.00, '2025-03-01', 'ACCEPTED'),   -- CloudNine Ltd (user_id 16)
  (3, 'QOT-003', 14, 18600.00, '2025-02-28', 'EXPIRED'),    -- DataDriven LLC (user_id 14)
  (4, 'QOT-004', 17, 27600.00, '2025-04-01', 'DRAFT'),      -- UrbanRetail (user_id 17)
  (5, 'QOT-005', 18, 20400.00, '2025-03-20', 'SENT'),       -- GreenLeaf Farms (user_id 18)
  (6, 'QOT-006', 13, 10800.00, '2025-03-10', 'REJECTED');   -- BuildRight Co (user_id 13)
...
VALUES
  -- QOT-001: NovaTech — ERP Suite Enterprise + CRM Pro Business
  (1, 1, 3, 1, 2400.00),
  (1, 2, 5, 1, 1200.00),
  -- QOT-002: CloudNine — HR Management Premium
  (2, 3, 7, 1,  900.00),
  -- QOT-003: DataDriven — CRM Pro Starter + Marketing Hub Growth
  (3, 2, 4, 1,  800.00),
  (3, 6, 13, 1, 850.00),
  -- QOT-004: UrbanRetail — Inventory Control Multi + Accounting Plus Advanced
  (4, 5, 11, 1, 1200.00),
  (4, 4, 9,  1, 1500.00),
  -- QOT-005: GreenLeaf — ERP Suite Professional
  (5, 1, 2, 1, 1700.00),
  -- QOT-006: BuildRight — Accounting Plus Standard
  (6, 4, 8, 1, 900.00);

-- ============================================================
-- 15. DISCOUNTS (matches mock discounts array)
-- ============================================================
...
-- ============================================================
-- 16. TAX RULES (matches mock taxRules array)
-- ============================================================
...
-- ============================================================
-- SEED DATA COMPLETE ✅
-- Database is ready for development & demo
-- ============================================================
